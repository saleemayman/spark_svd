...
echo ""
echo "run, cores_per_exec, partitions" > $CASE
join <(grep '\--executor-cores' $RESULT_FILES | awk -F':' -v var1=$PREFIX_SIZE1 -v var2=$PREFIX_SIZE2 '{print substr($1, var1, 2)", " substr($2, var2, 2)}' | sort) <(grep '2. RDD num. of Partitions' $RESULT_FILES | awk -F': ' -v var=$PREFIX_SIZE1 '{print substr($1, var, 2)", " $2}' | sort) -t$', ' >> $CASE
